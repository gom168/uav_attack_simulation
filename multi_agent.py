...
class MultiBattlefieldEnv:
    ...
    def _get_blue_deployment(self, battlefield_id: int) -> Dict[str, int]:
        ...
        # 动态调整最大出动量（避免前期倾巢而出）
        max_recon = max(min_recon, self.blue_uavs['recon'])
        max_attack = max(min_attack, self.blue_uavs['ground_attack'])
        max_escort = self.blue_uavs['escort']
        ...
        # 确保不超过剩余兵力
        return {
            'recon': min(recon, max_recon),
            'ground_attack': min(attack, max_attack),
            'escort': min(escort, max_escort)
        }
    ...
    def step(self, actions: List[Dict]):
        ...
        for i, action in enumerate(actions):
            if not self._is_battlefield_active(i):  # Only deploy to inactive battlefields
                ...
                recon = max(1, min(action.get('recon', 0), self.red_uavs['recon']))
                interceptor = max(1, min(action.get('interceptor', 0), self.red_uavs['interceptor']))
                escort = min(action.get('escort', 0), self.red_uavs['escort'])

                if recon > 0 and interceptor > 0:
        ...
        self.global_done = (self.red_uavs['interceptor'] <= 0 or self.red_uavs['recon'] <= 1 or self.blue_uavs['recon'] <= 1 or
                            (self.blue_uavs['ground_attack'] <= 1 or self.blue_uavs['escort'] <= 0))
